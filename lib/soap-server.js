...
SoapServer.prototype.castArgument = function(inputName, requestArgument, options){
	...
	var value = ('object' === typeof requestArgument) ? requestArgument[0]._ : requestArgument;

	switch(options.type){
		case 'number':
			var requestInteger = Number(value);
			if((requestInteger + '') != value)
				throw new SoapServiceException(SoapServiceException.INVALID_INPUT_TYPE, 'Invalid input [' + inputName + '] type ' + typeof value + ', expected ' + options.type);
...
SoapServer.prototype.handleOperationRequest = function(service, soapRequest){
  ...
  var operationRequest = [];
	...
	for( var m1 in  soapRequest) {

			if( m1.match(/.*[:]Envelope/)!==null ) {

					for( var m2 in soapRequest[m1] ) {

							if( m2.match(/.*[:]Body/)!==null ) {
									operationRequest = soapRequest[m1][m2][0];
									break;
							}
					}
					break;
	...
	if (req.method == 'POST') {
		...
		req.on('end', function () {
			...
			parseXml(body, function (err, soapRequest) {
				...
				//responseXml += '	xmlns:soap="http://www.w3.org/2003/05/soap-envelope"\n';
				//responseXml += '	soap:encodingStyle="http://www.w3.org/2003/05/soap-encoding">\n';
				responseXml += '	xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"\n';
				responseXml += '	soap:encodingStyle="http://schemas.xmlsoap.org/soap/encoding">\n';
...
SoapServer.prototype.getWsdl = function(host, service){
	...
	for(operationName in service.operations){
		...
		wsdl += '	<message name="' + operationName + 'Request">\n';
		for(var inputName in operation.inputs){
			var input = operation.inputs[inputName];
			if(input.type == 'object'){
